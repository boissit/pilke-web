...
`screenshotFeedback.yaml` the ones needing a fixture that flow cannot build. They are
copied here by hand from wherever Maestro wrote them — current versions put a
`takeScreenshot: shots/x` under
`~/.maestro/tests/<run>/<flow name>/takeScreenshot/shots/x.png` rather than in the
directory the command was run from, so check there before concluding a flow wrote
nothing.

⚠️ **Run the flows against a preview build** (`npm run build:ci`), which is what CI does.
A development build adds two things a marketing asset must not carry: the dev launcher
appears after every `clearState: true` unless the flow reconnects it, and LogBox parks a
red warning toast over the bottom of the screen — it landed in a `date` and a `feedback`
capture during the #72 reshoot and both had to be retaken.
